...
LOG_TEMPLATES = {
    "nginx": {
        ...
        "generator": lambda rng=random: {
            "timestamp": f"{rng.randint(1,28):02d}/{'Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec'.split()[rng.randint(0,11)]}/{2024}:{rng.randint(0,23):02d}:{rng.randint(0,59):02d}:{rng.randint(0,59):02d} +0000",
    ...
    "apache": {
        ...
        "generator": lambda rng=random: {
            "timestamp": f"{'Sun Mon Tue Wed Thu Fri Sat'.split()[rng.randint(0,6)]} {'Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec'.split()[rng.randint(0,11)]} {rng.randint(1,28):02d} {rng.randint(0,23):02d}:{rng.randint(0,59):02d}:{rng.randint(0,59):02d} {2024}",
    ...
    "json_app": {
        "pattern": None,
        "generator": lambda rng=random: {
            "timestamp": f"{2024}-{rng.randint(1,12):02d}-{rng.randint(1,28):02d}T{rng.randint(0,23):02d}:{rng.randint(0,59):02d}:{rng.randint(0,59):02d}.{rng.randint(0,999):03d}Z",
    ...
    "custom": {
        "pattern": None,
        "generator": lambda rng=random: {
            "timestamp": str(1704067200 - rng.randint(0, 86400)),  # 固定基准（2024-01-01），保证种子数据可复现
...
def build_log_entries(log_type: str, count: int, rng: random.Random = random):
    """按给定随机源生成日志（种子固定时输出可复现，供大屏口径快照使用）。"""
...
def generate_logs(req: GenerateRequest):
    logs = build_log_entries(req.type, req.count)
...
def analyze_logs(logs_data, rules, query, now_str=None):
    now_str = now_str or time.strftime("%H:%M:%S")
    logs = logs_data
    n = len(logs)
...
# 大屏数据权限路由（放文件末尾注册：screen 模块反向复用本文件的日志生成/分析能力）
